    ...
    def _gumbel(self, logits, temperature: float,
        ...
        if hard:
            ...
            argmax_mask = tf.one_hot(argmax_index, tf.shape(sample)[-1],
                                     axis=-1)
            sample = tf.where(argmax_mask == 1.0, sample,
                              tf.zeros_like(sample, dtype=tf.float32))
        return sample
...
class R2NReasoningLayer(ReasoningLayer):
    ...
    def call(self, inputs, **kwargs):
        ...
        predictions = tf.expand_dims(self.output_layer(atom_embeddings),
                                     axis=-1)
...
class DCRReasoningLayer(ReasoningLayer):
    ...
    def call(self, inputs, **kwargs):
        ...
        for rule in self.templates:
            ...
            if self._explain_mode:
                explain_info[rule.name] = list(self.rule_embedders[rule.name](
                    [concepts, atom_embeddings],
                    return_attn=True)) + [A_in, A_out]
        ...
        if self._explain_mode:
            return predictions, input_atom_embeddings, explain_info
        else:
            return predictions, input_atom_embeddings

    def explain(self, inputs):
        self._explain_mode = True
        _, _, explain_info = self(inputs)
        explanations = {r:self.rule_embedders[r].explain(*info)
                        for r, info in explain_info.items()}
...
class ClusteredDCRReasoningLayer(DCRReasoningLayer):
    ...
    def __init__(self,
        ...
        for rule in templates:
            ...
            self.formula_embedders[rule.name] = tf.keras.Sequential([
                Dense(num_formulas * formula_hidden_size, activation=None),
                LeakyReLU(),
                Dense(num_formulas * formula_hidden_size, activation=None,
    ...
    #       # Shape (batch_size, num_formulas * formula_embedding_size)
    #       formula_embeddings = self.formula_embedders[rule.name](
    #           atom_embeddings)
    #       # Shape (batch_size, num_formulas, formula_embedding_size)
    #       formula_embeddings = tf.reshape(
    #           formula_embeddings, shape=(tf.shape(atom_embeddings)[0],
    #                                      self.num_formulas,
    #                                      self.formula_hidden_size))
    ...
    #       # Shape (batch_size, num_formulas, 1)
    #       formula_prob = tf.expand_dims(tf.nn.softmax(formula_logits),
    #                                      axis=-1)
    #       # Shape (batch_size, num_formulas, formula_embedding_size)
    #       formula_prob = tf.tile(
    #           formula_prob,
    #           multiples=(1, 1, self.formula_hidden_size))

    #       # Shape (batch_size, formula_embedding_size)
    #       formula_embeddings = tf.reduce_sum(
    #           formula_embeddings * formula_prob, axis=1)
    #       # Shape (batch_size, num_body_atoms, formula_embedding_size
    ...
    def call(self, inputs, **kwargs):
        ...
        for rule in self.templates:
            ...
            formula_embeddings = tf.reshape(
                formula_embeddings,
                shape=(-1, self.num_formulas, self.formula_hidden_size))
            ...
            #formula_mask = tf.one_hot(formula_selected, tf.shape(formula_selected)[1], axis=-1)
            #formula_embeddings = tf.boolean_mask(formula_mask == 1.0, formula_embeddings)
            ...
            if self._explain_mode:
               explain_info[rule.name] = list(rule_embedders(
                   [concepts, formula_embeddings],
                   return_attn=True)) + [A_in, A_out]
...
class _GatedSBRReasoningLayerBase(SBRReasoningLayer):
    ...
    def __init__(self, rules: List[Rule],
        ...
        super().__init__(rules, aggregation_type, logic, **kwargs)
...
class GatedSBRReasoningLayer(_GatedSBRReasoningLayerBase):
    ...
    def __init__(self, rules: List[Rule],
        ...
        super().__init__(rules, aggregation_type, regularization, logic,
                         per_grounding_gate=True, **kwargs)
...
class RNMReasoningLayer(_GatedSBRReasoningLayerBase):
    ...
    def __init__(self, rules: List[Rule],
        ...
        super().__init__(rules, aggregation_type, regularization, logic,
                         per_grounding_gate=False, **kwargs)
...
class DeepLogicModelReasoner(Layer):
    ...
    def __init__(self, semantic_loss, output_layer, update_model, **kwargs):
