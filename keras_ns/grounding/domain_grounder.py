...
from keras_ns.logic.commons import Rule,  Atom, Domain, RuleGroundings
...
class DomainFullGrounder(Engine):
    ...
    def ground(self,
        ...
        if self.restrict_to_query_constants:
            constants_to_consider = set()
            for q in queries:
                constants_to_consider.update(set(q[1:]))
        ...
        for rule in self.rules:
            ...
            for ground_vars in product(
                *[self.domains[d].constants for d in rule.vars2domain.values()]):
                if self.restrict_to_query_constants:
                    if len(set(ground_vars).intersection(
                        constants_to_consider)) == 0:
                        continue
...
class DomainBodyGrounder(Engine):

    def __init__(self, domains:Dict[str, Domain], rules: List[Rule],
                 exclude_symmetric: bool=False,
                 exclude_query: bool=False,
                 limit: int=None):
        self.rules = rules
    ...
    def ground_one_rule(self, rule: Rule, queries: List[Tuple]) -> Union[
      ...
      for i,q in enumerate(queries):
        ...
        for ground_vars in product(
            *[self.domains[d].constants for d in vars_to_ground.values()]):
