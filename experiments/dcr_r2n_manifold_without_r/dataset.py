...
class CollectiveDataHandler():
    ...
    def __init__(self, dataset_name,
        ...
        self.predicates = [Predicate(p,[self.domains[0] for _ in range(a)])
                           for p,(id,a) in predicates]
        ...
        self.classes = [line.strip().split(",") for line in open(classes_path).readlines()]
    ...
    def get_dataset(self, split:str):
        ...
        for t in facts:
            # if len(t) == 2: # unary
                c = t[1]
                exclusive_group = self.classes[0] if t[0] in self.classes[0] else self.classes[1]
                queries.append([])
                labels.append([])
                for r in exclusive_group:
                    query = (r,c)
                    label = int(r == t[0])
