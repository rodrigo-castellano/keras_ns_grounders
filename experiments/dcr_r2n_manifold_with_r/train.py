...
def main(base_path, output_filename, kge_output_filename, log_filename, args):
    ...
    if args.reasoner_depth > 0 and args.num_rules > 0:
        ...
        rules = [Rule('r0', body_atoms=[("class", "X", str(c)) for c in fol.domains[1].constants] + \
                                        [("r", v[0], v[1]) for v in [("X", "Y"), ("Y", "X")]],
                            head_atoms=[("class", "Y", str(c)) for c in fol.domains[1].constants] ,
        ...
        engine = ns.grounding.DomainGrounder(domains={d.name: d for d in fol.domains}, rules=rules)
    else:
        rules = []
        engine = None
