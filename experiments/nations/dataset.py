...
def GetPredicate2Domains(
    atoms: List[Tuple[str, str, str]],
    constant2domain: Dict[str, str]) -> Dict[str, Tuple[str, str]]:
    predictate2domains = {}
    for a in atoms:
        p = a[0]
        for c in a[1:]:
        ...
        if p in predictate2domains:
          assert (domain_tuple == predictate2domains[p]), (
              'Atom %s Domains differ for same constants %s!=%s' % (
                  str(a), str(domain_tuple), str(predictate2domains[p])))
        else:
...
class KGCDataHandler():
    ...
    def __init__(self, dataset_name,
        ...
        self.default_domain_name = name + '__default__'
        self.domains = [Domain(self.default_domain_name, self.constants)]
        # Predicates are assigned to the global domain. This is because
        # predicates could have mixed domains in some datasets.
        ...
        if domain_file is not None:
            ...
            self.domains += [
                Domain(domain_name, constants)
                for domain_name,constants in self.domain2constants.items()]
            # domain_name2domain = {domain.name:domain for domain in self.domains}
            #predicate2domains = GetPredicate2Domains(
            #    atoms=list(self.ground_facts_set),
            #    constant2domain=self.constant2domain)
        else:
            self.constant2domain = {c:self.default_domain_name
                                    for c in self.constants}
            self.domain2constants = {self.default_domain_name: self.constants}
