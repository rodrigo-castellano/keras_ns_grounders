...
def train_and_validate(cfg, model, train_data, valid_data, device, logger, filtered_data=None, batch_per_epoch=None):
    ...
    for i in range(0, cfg.train.num_epoch, step):
        print('Epoch',i, 'of',cfg.train.num_epoch, '. Step:',step)
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in islice(train_loader, batch_per_epoch):
                # print the current batch number if it is more of 10% of the total number of batches
                print('batch',batch_id, 'of',batch_per_epoch) if batch_id % (batch_per_epoch//10) == 0 else None
                # batch goes from shape (batchsize,3) to (batchsize, num_negatives+1=257, 3). Corruptions only in tail
                batch = tasks.negative_sampling(train_data, batch, cfg.task.num_negative,
                                                strict=cfg.task.strict_negative)
                # print('batch with negatives',cfg.task.strict_negative,batch.shape, batch[0])
...
def test(cfg, model, test_data, device, logger, filtered_data=None, return_metrics=False):
    ...
    for i,batch in enumerate(test_loader):
        ...
        # batch=[batch_size=16=queries, 3] (h,r,t) for each of the 16 heads, 1594 negative tails are sampled. The last dimension is (h, r, t)
...
if __name__ == "__main__":
    ...
    if task_name == "InductiveInference":
        ...
        # Filtered data is data that is used to check that in the corruptions there are no true triples. In transductive it is the whole dataset. 
        # In inductive, depending on the dataset, it is the training set, the validation set, or the test set.
    ...
    # print('\ntrain_data')
    # print('edge_index',train_data.edge_index.shape, train_data.edge_index)
    # print('edge_type',train_data.edge_type.shape, train_data.edge_type)
    # print('num_nodes',train_data.num_nodes)
    # print('target_edge_index',train_data.target_edge_index.shape, train_data.target_edge_index)
    # print('target_edge_type',train_data.target_edge_type.shape, train_data.target_edge_type)
    # print('num_relations',train_data.num_relations)
    # print('\nvalid_data')
    # print('edge_index',valid_data.edge_index.shape, valid_data.edge_index)
    # print('edge_type',valid_data.edge_type.shape, valid_data.edge_type)
    # print('num_nodes',valid_data.num_nodes)
    # print('target_edge_index',valid_data.target_edge_index.shape, valid_data.target_edge_index)
    # print('target_edge_type',valid_data.target_edge_type.shape, valid_data.target_edge_type)
    # print('num_relations',valid_data.num_relations)
    # print('\ntest_data')
    # print('edge_index',test_data.edge_index.shape, test_data.edge_index)
    # print('edge_type',test_data.edge_type.shape, test_data.edge_type)
    # print('num_nodes',test_data.num_nodes)
    # print('target_edge_index',test_data.target_edge_index.shape, test_data.target_edge_index)
    # print('target_edge_type',test_data.target_edge_type.shape, test_data.target_edge_type)
    # print('num_relations',test_data.num_relations)
    # print('\nfiltered_data')
    # print('edge_index',filtered_data.edge_index.shape, filtered_data.edge_index) # edge_index is the sum of all target_edge_index
    # print('edge_type',filtered_data.edge_type.shape, filtered_data.edge_type) # same for edge_type
    # print('num_nodes',filtered_data.num_nodes) # num_nodes and num_relations are the same for all datasets
