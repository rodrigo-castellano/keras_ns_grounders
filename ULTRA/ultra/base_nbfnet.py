...
class BaseNBFNet(nn.Module):
    ...
    def remove_easy_edges(self, data, h_index, t_index, r_index=None):
        ...
        r_index_ext = torch.cat([r_index, r_index + torch.div(data.num_relations, 2, rounding_mode='floor')], dim=-1)
    ...
    def negative_sample_to_tail(self, h_index, t_index, r_index, num_direct_rel):
        # In this function, if the corruption is head, we convert it to tail corruption by inverting the relations (and adding them as new relations)
        # and if the corruption is tail, we leave it as it is
        # convert p(h | t, r) to p(t' | h', r')
        # h' = t, r' = r^{-1}, t' = h
        # print('h_index', h_index)
        # print('t_index', t_index)
        # print('r_index', r_index)
        # print('h_index[:, [0]]',h_index[:, [0]])
        # print('h_index == h_index[:, [0]]',h_index == h_index[:, [0]])
        # print(' (h_index == h_index[:, [0]]).all(dim=-1, keepdim=True)', (h_index == h_index[:, [0]]).all(dim=-1, keepdim=True))
        # Here it check whether for each query, the head entity is the same in the rest of the corruptions (to verify if it is head or tail corruption)
        # is_t_neg checks if that condition is fulfilled in all the negatives
        is_t_neg = (h_index == h_index[:, [0]]).all(dim=-1, keepdim=True)
        new_h_index = torch.where(is_t_neg, h_index, t_index)
        new_t_index = torch.where(is_t_neg, t_index, h_index)
        new_r_index = torch.where(is_t_neg, r_index, r_index + num_direct_rel)
        # print('new_h_index', new_h_index)
        # print('new_t_index', new_t_index)
        # print('new_r_index', new_r_index)
