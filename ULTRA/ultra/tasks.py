...
def compute_ranking(pred, target, mask=None):
    ...
    if mask is not None:
        # filtered ranking
        ranking = torch.sum((pos_pred <= pred) & mask, dim=-1) + 1 # this +1 is because the mask doesnt include the target (true value)
    else:
        # unfiltered ranking
...
def build_relation_graph(graph):
    ...
    # print('\n')
    # print('1edge_index', edge_index.shape, edge_index)
    # print('1edge_type', edge_type.shape, edge_type)
    # print('1num_nodes', num_nodes)
    # print('1num_rels', num_rels)
    # print('1Non unique Eh',torch.vstack([edge_index[0], edge_type]).T.shape)
    # print('edge_index[0]', edge_index[0].shape, edge_index[0][:200])
    # print('edge_type', edge_type.shape, edge_type[:200])
    # print('Eh non unique', torch.vstack([edge_index[0], edge_type]).T.shape, torch.vstack([edge_index[0], edge_type]).T[:200])
    Eh = torch.vstack([edge_index[0], edge_type]).T.unique(dim=0)  # (num_edges, 2) # Takes the uniques pairs of head and relation
    # print('1Eh', Eh.shape, Eh)
    # print('1Eh[:, 0]', Eh[:, 0].shape, Eh[:, 0])
    Dh = scatter_add(torch.ones_like(Eh[:, 1]), Eh[:, 0])
    # print('1Dh', Dh.shape, Dh)
