...
class GrailInductiveDataset(InMemoryDataset):
    ...
    def process(self):
        # it treats train and validation with the same vocab (train), but test with a different one
        # for the relation vocab, it uses a unique vocab
        ...
        # here, for every entity in head, tail, and also relations, creates an index. With those indeces, it creates a tensor of triplets
        ...
        edge_index = triplets[:, :2].t() # For all the queries, this takes in the first dim the head, and in the second the tail
        edge_type = triplets[:, 2] # For all the queries, it takes the relation
        ...
        # num_samples has the cumulative total number of samples in each txt_file
        train_fact_slice = slice(None, sum(num_samples[:1]))
        test_fact_slice = slice(sum(num_samples[:2]), sum(num_samples[:3]))
        train_fact_index = edge_index[:, train_fact_slice] # [[heads],[tails]] of the train set
        train_fact_type = edge_type[train_fact_slice] # [relations] of the train set
...
class TransductiveDataset(InMemoryDataset):
    ...
    def process(self):
        # here it uses one vocab for all the splits
        train_files = self.raw_paths[:3]
        ...
        num_node = test_results["num_node"] # total number of entities in all splits
        # the same for rels: in most cases train == test for transductive
        # for AristoV4 train rels 1593, test 1604
        num_relations = test_results["num_relation"] # total number of relations in all splits
        ...
        # edge_index, edge_type are the same for all splits (the ones from train, which are the ones in all the splits, 
        # since it is transductive). Then, for target, it uses the edges and types of the split 
...
class CustomTransductive(InMemoryDataset):
    delimiter = None
    ...
    def __init__(self, root, transform=None, pre_transform=build_relation_graph, **kwargs):
        ...
        # path_countries_root = 'C:\\Users\\rodri\\Downloads\\PhD_code\\Review_grounders\\keras_ns_grounders\\experiments\\data\\FB15k237_triplets\\'
        # self.raw_paths = [
        #     path_countries_root + 'train.txt',
        #     path_countries_root + 'valid.txt',
        #     path_countries_root + 'test.txt'    
        # ]
        # self.root = root
    ...
    def process(self):
        ...
        train_triplets = train_results["triplets"]
        # print('entity_vocab',train_results["inv_entity_vocab"])
        ...
        train_edges = torch.cat([train_target_edges, train_target_edges.flip(0)], dim=1)
        train_etypes = torch.cat([train_target_etypes, train_target_etypes+num_relations])
        # Edge_index is the head and tail of the edges, e.g. (2,1109). it takes all the triplets, flips the heads and tails, and concatenates them
        # At the end the final shape is (2,2218) = (head-tail,n_triples)+(tail-head,n_triples): ALL THE TRIPLETS + INV TRIPLETS
        # Edge_type is the relation of the edges, e.g. 0. it takes all the relations, flips them and concatenates them
        train_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_relations*2)
        valid_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=valid_edges, target_edge_type=valid_etypes, num_relations=num_relations*2)
...
class CountriesTransductive(CustomTransductive):
    name = "countries"

class FB15k237Transductive(CustomTransductive):
    name = "fb15k237"

class FB15k237FullTransductive(CustomTransductive):
    name = "fb15k237_full"

class KinshipTransductive(CustomTransductive):
    name = "kinship"

class KinshipFullTransductive(CustomTransductive):
    name = "kinship_full"
