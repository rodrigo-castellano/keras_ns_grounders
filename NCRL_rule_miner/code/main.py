...
def test(args, dataset):
    ...
    for epoches in range(n_epoches):
        ...
        print("## body {} / {}".format((epoches+1)* batch_size, (n_epoches+1)* batch_size))
...
if __name__ == '__main__':
    ...
    dataset = Dataset(data_root=data_path, sparsity=args.sparsity, inv=False)
    ...
    if args.test:
        ...
        if args.get_rule:
            ...
            for rule_len in rule_conf:
                rule_path = "./rules/{}_{}_{}.txt".format(args.output_file, args.topk, rule_len)
                print("\nrule length:{}".format(rule_len))
